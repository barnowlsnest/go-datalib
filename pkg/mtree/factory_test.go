...
func (s *NewNodeFactoryTestSuite) TestNode() {
	factory := NewFactory[int](testMaxBreadth, testMaxDepth, serial.Seq())
	n1 := factory.Node(DefaultShard, ValueOpt[int](10))
	s.NotNil(n1)
	n2 := factory.Node(DefaultShard, ValueOpt[int](20))
	s.NotNil(n2)
	n3 := factory.Node(DefaultShard, ValueOpt[int](30), ChildOpt[int](n1), ChildOpt[int](n2))
	s.NotNil(n3)
...
func (s *NewNodeFactoryTestSuite) TestRoot() {
	factory := NewFactory[int](testMaxBreadth, testMaxDepth, serial.Seq())
	rootNode := factory.Root(DefaultShard, 50)
	s.NotNil(rootNode)
...
func (s *NewNodeFactoryTestSuite) TestRootWithChildren() {
	factory := NewFactory[int](testMaxBreadth, testMaxDepth, serial.Seq())
	n1 := factory.Node(DefaultShard, ValueOpt[int](1))
	n2 := factory.Node(DefaultShard, ValueOpt[int](2))
	n3 := factory.Node(DefaultShard, ValueOpt[int](3))
	r, err := factory.RootWithChildren(DefaultShard, 100, n1, n2, n3)
	s.NoError(err)
	s.NotNil(r)
